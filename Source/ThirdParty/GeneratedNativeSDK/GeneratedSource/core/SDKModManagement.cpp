...
#include "modio/detail/ops/AddTagsOp.h"
#include "modio/detail/ops/DeleteModFileOp.h"
#include "modio/detail/ops/DeleteTagsOp.h"
#include "modio/detail/ops/EditModOp.h"
#include "modio/detail/ops/GetUserMods.h"
...
namespace Modio
{
	...
	void AddTagsToModAsync(Modio::ModID ModToSubscribeTo, std::vector<std::string> Tags,
						   std::function<void(Modio::ErrorCode)> OnComplete)
	{
		if (Modio::Detail::RequireSDKIsInitialized(OnComplete) && Modio::Detail::RequireNotRateLimited(OnComplete) &&
			Modio::Detail::RequireUserIsAuthenticated(OnComplete))
		{
			return asio::async_compose<std::function<void(Modio::ErrorCode)>, void(Modio::ErrorCode)>(
				Modio::Detail::AddTagsOp(ModToSubscribeTo, Modio::Detail::SDKSessionData::CurrentAPIKey(), Tags),
				OnComplete, Modio::Detail::Services::GetGlobalContext().get_executor());
		}
	}
	void DeleteTagsModAsync(Modio::ModID ModId, std::vector<std::string> Tags,
							std::function<void(Modio::ErrorCode)> OnComplete)
	{
		if (Modio::Detail::RequireSDKIsInitialized(OnComplete) && Modio::Detail::RequireNotRateLimited(OnComplete) &&
			Modio::Detail::RequireUserIsAuthenticated(OnComplete))
		{
			return asio::async_compose<std::function<void(Modio::ErrorCode)>, void(Modio::ErrorCode)>(
				Modio::Detail::DeleteTagsOp(ModId, Modio::Detail::SDKSessionData::CurrentAPIKey(), Tags), OnComplete,
	...
	void EditModAsync(Modio::ModID ModID, Modio::CreateModParams Params, std::function<void(Modio::ErrorCode)> Callback)
	{
		if (Modio::Detail::RequireSDKIsInitialized(Callback) && Modio::Detail::RequireUserIsAuthenticated(Callback))
		{
			return asio::async_compose<std::function<void(Modio::ErrorCode)>, void(Modio::ErrorCode)>(
				Modio::Detail::EditModOp(ModID, Params), Callback,
				Modio::Detail::Services::GetGlobalContext().get_executor());
		}
	}
	...
	void GetUserModsAsync(std::function<void(Modio::ErrorCode, Modio::Optional<Modio::ModInfoList>)> Callback)
	{
		if (Modio::Detail::RequireSDKIsInitialized(Callback) && Modio::Detail::RequireUserIsAuthenticated(Callback))
		{
			return Modio::Detail::GetUserModsOpAsync(Callback);
		}
	}
} // namespace Modio